...
def compute_actuation(t: ti.i32):
    # TODO modified as our own actuation function
    for i in range(n_actuators):
        ...
        print(act)
        act += bias[i]
        actuation[t, i] = ti.tanh(act) #mapping to actuation of joint i at time t within -1,+1
...
@ti.kernel
def compute_loss():
    #encourage as much distance as possible
    dist = x_avg[None][0]
    loss[None] = -dist
...
# TODO forward dynamics function
...
class Scene:
    ...
    def add_rect(self, x, y, w, h, actuation, ptype=1):
        #left down point(x,y) and (w,h) of rec
        if ptype == 0:
            assert actuation == -1
        global n_particles
        w_count = int(w / dx) * 2
        ...
        for i in range(w_count):
            for j in range(h_count):
                ...
                self.actuator_id.append(actuation) #actuation per particle (element) keep record of actuator index
...
def robot(scene):
    ...
    #TODO Modify this to change robot basic config
    scene.set_offset(0.1, 0.03)
    scene.add_rect(0.0, 0.1, 0.3, 0.1, -1)#body

    scene.add_rect(0.0, 0.0, 0.05, 0.05, 0)#left leg left
    scene.add_rect(0.0, 0.05, 0.05, 0.05, 4)

    scene.add_rect(0.05, 0.0, 0.05, 0.05, 1)#left leg left
    scene.add_rect(0.05, 0.05, 0.05, 0.05, 5)

    scene.add_rect(0.2, 0.0, 0.05, 0.05, 2)#right leg left
    scene.add_rect(0.2, 0.05, 0.05, 0.05, 6)

    scene.add_rect(0.25, 0.0, 0.05, 0.05, 3)#right leg right
    scene.add_rect(0.25, 0.05, 0.05, 0.05, 7)
